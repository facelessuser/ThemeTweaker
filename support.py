...
CSS = '''
div.theme-tweaker { padding: 10px; margin: 0; }
.theme-tweaker h1, .theme-tweaker h2, .theme-tweaker h3,
.theme-tweaker h4, .theme-tweaker h5, .theme-tweaker h6 {
    {{'.string'|css}}
}
.theme-tweaker blockquote { {{'.comment'|css}} }
.theme-tweaker a { text-decoration: none; }
'''
...
class ThemeTweakerSupportInfoCommand(sublime_plugin.ApplicationCommand):
    """Support info."""
...
class ThemeTweakerOpenSiteCommand(sublime_plugin.ApplicationCommand):
    """Open site links."""
...
class ThemeTweakerDocCommand(sublime_plugin.WindowCommand):
    """Open doc page."""
    ...
    def run(self, page):
        ...
        if not has_phantom_support:
            ...
            view.set_name('ThemeTweaker - Quick Start')
...
class ThemeTweakerChangesCommand(sublime_plugin.WindowCommand):
    """Changelog command."""
    ...
    def run(self):
        ...
        text = sublime.load_resource('Packages/ThemeTweaker/CHANGES.md')
        view = self.window.new_file()
        view.set_name('ThemeTweaker - Changelog')
        view.settings().set('gutter', False)
        view.settings().set('word_wrap', False)
        if has_phantom_support:
            mdpopups.add_phantom(
